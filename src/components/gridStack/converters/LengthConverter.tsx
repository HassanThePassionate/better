...
const lengthUnits: Unit[] = [
  ...
  { name: "Decimeter", symbol: "dm", conversionFactor: 10 },
  { name: "Centimeter", symbol: "cm", conversionFactor: 100 },
  { name: "Millimeter", symbol: "mm", conversionFactor: 1000 },
  { name: "Micrometer", symbol: "μm", conversionFactor: 1000000 },
  { name: "Nanometer", symbol: "nm", conversionFactor: 1000000000 },
  { name: "Picometer", symbol: "pm", conversionFactor: 1000000000000 },
  { name: "Inch", symbol: "in", conversionFactor: 39.3701 },
  { name: "Milli-inch", symbol: "mil", conversionFactor: 39370.1 },
  ...
  { name: "Nautical mile", symbol: "nmi", conversionFactor: 0.000539957 },
];

export default function LengthConverter() {
  const [baseValue, setBaseValue] = useState<number>(1);
  const [baseUnit, setBaseUnit] = useState<Unit>(lengthUnits[0]);
  ...
  const calculateConversions = () => {
    ...
    lengthUnits.forEach((unit) => {
      // Convert base value to meters first, then to target unit
      const valueInMeters = baseValue / baseUnit.conversionFactor;
      newValues[unit.symbol] = valueInMeters * unit.conversionFactor;
    });
