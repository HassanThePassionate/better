...
export default function LargeCryptoWidget() {
  ...
  const [chartSize, setChartSize] = useState({ width: 260, height: 150 });
  ...
  useEffect(() => {
    const updateSize = () => {
      ...
      if (screenWidth > 1600) {
        setChartSize({ width: 390, height: 305 });
      } else {
        setChartSize({ width: 260, height: 150 });
      }
    };
  ...
  // ===== RENDER =====
  return (
    <div className='w-full h-full bg-card text-text rounded-[20px] overflow-hidden p-3 border-0 '>
      {/* Header */}
          ...
          {tooltip.visible && hasValidData && !isRefreshing && (
            <div
              className={`absolute bg-card text-text text-xs py-1 px-2 rounded pointer-events-none z-50 font-medium shadow-lg transition-all duration-100 ease-out border ${
                isPriceUp ? "border-[#22c55e]" : "border-errortext-error"
              }`}
              style={{
                left: `${tooltip.x}px`,
                top:
